...
class _News
{
	protected $_data = array('id' => null, 'date' => null, 'user_id' => null, 'title' => null, 'text' => null);
	...
	public static function find($id)
	{
		...
		$req = mysql_query('SELECT * FROM news WHERE id=' . $id) or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new News(mysql_fetch_array($req));
		return false;
	}
	...
	{
		return News::search(array(), $order);
	}
	...
	{
		$req = mysql_query('SELECT * FROM news WHERE ' . $field . ' LIKE "' . $value . '"') or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new News(mysql_fetch_array($req));
		return false;
	}
	...
	public static function search($criteria = array(), $order="", $limit=false, $onlyCount=false)
	{
		...
		if (!$onlyCount)
			...
			$query = "SELECT COUNT(1) AS count FROM news";
	...
	{
		return News::search(array(), "", 1, true);
	}
