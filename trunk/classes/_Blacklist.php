...
class _Blacklist
{
	...
	public static function find($id)
	{
		...
		$req = mysql_query('SELECT * FROM blacklist WHERE id=' . $id) or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Blacklist(mysql_fetch_array($req));
		return false;
	}
	...
	{
		return Blacklist::search(array(), $order);
	}
	...
	{
		$req = mysql_query('SELECT * FROM blacklist WHERE ' . $field . ' LIKE "' . $value . '"') or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Blacklist(mysql_fetch_array($req));
		return false;
	}
	...
	public static function search($criteria = array(), $order="", $limit=false, $onlyCount=false)
	{
		...
		if (!$onlyCount)
			...
			$query = "SELECT COUNT(1) AS count FROM blacklist";
		...
		while ($res = mysql_fetch_array($req))
			$results [$res['id']] = new Blacklist($res);
	...
	{
		return Blacklist::search(array(), "", 1, true);
	}
