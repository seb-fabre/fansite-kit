...
class _Test
{
	...
	public static function find($id)
	{
		...
		$req = mysql_query('SELECT * FROM test WHERE id=' . $id) or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Test(mysql_fetch_array($req));
		return false;
	}
	...
	{
		return Test::search(array(), $order);
	}
	...
	{
		$req = mysql_query('SELECT * FROM test WHERE ' . $field . ' LIKE "' . $value . '"') or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Test(mysql_fetch_array($req));
		return false;
	}
	...
	public static function search($criteria = array(), $order="", $limit=false, $onlyCount=false)
	{
		...
		if (!$onlyCount)
			...
			$query = "SELECT COUNT(1) AS count FROM test";
	...
	{
		return Test::search(array(), "", 1, true);
	}
