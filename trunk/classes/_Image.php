...
class _Image
{
	protected $_data = array('id' => null, 'name' => null, 'gallery_id' => null, 'views' => null, 'date' => null, 'user_id' => null, 'totalnotes' => null, 'totalvotes' => null);
	...
	public static function find($id)
	{
		...
		$req = mysql_query('SELECT * FROM image WHERE id=' . $id) or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Image(mysql_fetch_array($req));
		return false;
	}
	...
	{
		return Image::search(array(), $order);
	}
	...
	{
		$req = mysql_query('SELECT * FROM image WHERE ' . $field . ' LIKE "' . $value . '"') or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Image(mysql_fetch_array($req));
		return false;
	}
	...
	public static function search($criteria = array(), $order="", $limit=false, $onlyCount=false)
	{
		...
		if (!$onlyCount)
			...
			$query = "SELECT COUNT(1) AS count FROM image";
	...
	{
		return Image::search(array(), "", 1, true);
	}
	...
	{
		mysql_query("DELETE FROM image WHERE id=" . $this->id);
	}
