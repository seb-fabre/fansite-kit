...
class _Config
{
	...
	public static function find($id)
	{
		...
		$req = mysql_query('SELECT * FROM config WHERE id=' . $id) or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Config(mysql_fetch_array($req));
		return false;
	}
	...
	{
		$req = mysql_query('SELECT * FROM config WHERE ' . $field . ' LIKE "' . $value . '"') or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Config(mysql_fetch_array($req));
		return false;
	}
	...
	public static function search($criteria = array(), $order="", $limit=false, $onlyCount=false)
	{
		...
		if (!$onlyCount)
			...
			$query = "SELECT COUNT(1) AS count FROM config";
	...
	{
		return Config::search(array(), "", 1, true);
	}
