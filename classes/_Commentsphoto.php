<?php
class _Commentsphoto
{
	protected $_data = array('id' => null, 'comment' => null, 'user_id' => null, 'image_id' => null, 'date' => null);
	...
	public static function find($id)
	{
		...
		$req = mysql_query('SELECT * FROM commentsphoto WHERE id=' . $id) or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Commentsphoto(mysql_fetch_array($req));
		return false;
	}
	...
	{
		return Commentsphoto::search(array(), $order);
	}
	...
	{
		$req = mysql_query('SELECT * FROM commentsphoto WHERE ' . $field . ' LIKE "' . $value . '"') or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Commentsphoto(mysql_fetch_array($req));
		return false;
	}
	...
	public static function search($criteria = array(), $order="", $limit=false, $onlyCount=false)
	{
		...
		if (!$onlyCount)
			$query = "SELECT * FROM commentsphoto";
		else
			$query = "SELECT COUNT(1) AS count FROM commentsphoto";
		...
		while ($res = mysql_fetch_array($req))
			$results [$res['id']] = new Commentsphoto($res);
	...
	public function save()
	{
		...
		if ($this->_data['id'])
		{
			$query = 'UPDATE commentsphoto SET ';
		...
		{
			$query = 'INSERT INTO commentsphoto(';
			$glu = '';
			foreach ($this->_editedFields as $field)
			{
				$query .= $glu . $field;
	...
	{
		return Commentsphoto::search(array(), "", 1, true);
	}
	...
	{
		mysql_query("DELETE FROM commentsphoto WHERE id=" . $this->id);
	}
