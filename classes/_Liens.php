...
class _Liens
{
	...
	public static function find($id)
	{
		...
		$req = mysql_query('SELECT * FROM liens WHERE id=' . $id) or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Liens(mysql_fetch_array($req));
		return false;
	}
	...
	{
		return Liens::search(array(), $order);
	}
	...
	{
		$req = mysql_query('SELECT * FROM liens WHERE ' . $field . ' LIKE "' . $value . '"') or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Liens(mysql_fetch_array($req));
		return false;
	}
	...
	public static function search($criteria = array(), $order="", $limit=false, $onlyCount=false)
	{
		...
		if (!$onlyCount)
			...
			$query = "SELECT COUNT(1) AS count FROM liens";
	...
	{
		return Liens::search(array(), "", 1, true);
	}
