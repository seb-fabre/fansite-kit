...
class _Referrer
{
	...
	public static function find($id)
	{
		...
		$req = mysql_query('SELECT * FROM referrer WHERE id=' . $id) or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Referrer(mysql_fetch_array($req));
		return false;
	}
	...
	{
		return Referrer::search(array(), $order);
	}
	...
	{
		$req = mysql_query('SELECT * FROM referrer WHERE ' . $field . ' LIKE "' . $value . '"') or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new Referrer(mysql_fetch_array($req));
		return false;
	}
	...
	public static function search($criteria = array(), $order="", $limit=false, $onlyCount=false)
	{
		...
		if (!$onlyCount)
			...
			$query = "SELECT COUNT(1) AS count FROM referrer";
		...
		while ($res = mysql_fetch_array($req))
			$results [$res['id']] = new Referrer($res);
	...
	{
		return Referrer::search(array(), "", 1, true);
	}
