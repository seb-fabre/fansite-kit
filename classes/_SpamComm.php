...
class _SpamComm
{
	...
	public static function find($id)
	{
		...
		$req = mysql_query('SELECT * FROM spam_comm WHERE id=' . $id) or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new SpamComm(mysql_fetch_array($req));
		return false;
	}
	...
	{
		return SpamComm::search(array(), $order);
	}
	...
	{
		$req = mysql_query('SELECT * FROM spam_comm WHERE ' . $field . ' LIKE "' . $value . '"') or die(mysql_error());
		if (mysql_num_rows($req) != 0)
			return new SpamComm(mysql_fetch_array($req));
		return false;
	}
	...
	public static function search($criteria = array(), $order="", $limit=false, $onlyCount=false)
	{
		...
		if (!$onlyCount)
			$query = "SELECT * FROM spam_comm";
		else
			$query = "SELECT COUNT(1) AS count FROM spam_comm";
		...
		while ($res = mysql_fetch_array($req))
			$results [$res['id']] = new SpamComm($res);
	...
	{
		return SpamComm::search(array(), "", 1, true);
	}
